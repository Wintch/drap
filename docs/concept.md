...
1. **RBDOOM-3-BFG (NVRHI)** — DX12/Vulkan, native `com_engineHz`, a path to RT, most active mainline. Real VR work exists but lives on separate, unmerged branches (OpenVR-based) — see the "Current status" correction below; the `master` branch we built and tested is flat-desktop only.
...
**Where things actually stand, in one place:** four engines are now built and confirmed running with real content this session (TDM, Prey2006, RBDOOM-3-BFG, OpenJK) — but only **one** has ever been tested with the headset on: Quake3VR/Trinity VR. RBDOOM-3-BFG, TDM, and Prey2006's verification this week was flat-desktop rendering and frame-pacing only, not headset gameplay.

### ⚠️ Correction (2026-09-15): RBDOOM-3-BFG does have active, real VR work — it just isn't on the branch we built

Earlier this doc stated flatly that RBDOOM-3-BFG "has no VR layer, would have to be built from scratch." **That was wrong**, caught by checking the project's actual GitHub issues/branches instead of relying on the README alone. Verified directly against the repo:

- **The maintainer himself (RobertBeckebans) has been driving a real VR effort since at least Aug 2024** — issue #925 ("VR roadmap"). By June 2025 he called an OpenVR-based build "already playable with some minor bugs" and opened a public project board ("RBDOOM-VR 1.0") targeting Half-Life: Alyx-style controls.
- **The repo currently has multiple live VR branches** (as of Sept 2026): `openvr`, `openvr2` through `openvr5`, `openvr3-push-constants`, `legacy/openvr`, `mods/samsonvr`, `DOOM-3-BFG-VR-sync`. This is real, ongoing engineering, not an abandoned experiment.
- **Important caveat: it's OpenVR/SteamVR-based, not native OpenXR.** No OpenXR-titled branch or issue exists in the repo. That's a real integration question for our setup specifically, since `reverb-g2` talks to Monado directly via OpenXR with no SteamVR in the middle (unlike the mainline `master` branch we built and 120Hz-tested, which is plain flat-desktop code with none of this). Two realistic paths: run SteamVR-on-Linux against Monado (adds a layer we've deliberately avoided so far), or use an OpenVR→OpenXR shim (e.g. `OpenComposite`-style) to point the OpenVR calls at Monado directly, the same way we've been keeping everything else OpenXR-native.
- **What we actually tested was `master`**, which doesn't include any of this — so the 120Hz result stands (that's genuinely flat-desktop, unrelated to these branches), but "RBDOOM-3-BFG has no VR" as a blanket statement was incorrect. The real state is "VR exists on separate branches, not yet merged to stable, OpenVR not OpenXR, not yet tested by us."

**Also checked while correcting this: Doom3Quest's real lineage (Team-Beef-Studios, formerly DrBeef).** Confirmed via its own README: it is **not** based on BFG Edition at all — *"THIS PORT WILL NOT RUN WITH THE BFG EDITION, ONLY THE ORIGINAL DOOM 3 PK4 FILES WILL WORK."* It's forked from `d3wasm-multithread` (an Android/multithreaded fork of `d3wasm`, an Emscripten port of **dhewm3** — the original, non-BFG idTech4 GPL lineage). Real 6DoF via OpenGL ES MultiView single-pass stereo, ported Fully Possessed VR (IK arms, VR weapons, PDA UI), and — as of release v1.4.5 (March 2026) — a "Unified OpenXR build" covering Quest + Pico. Active as of Sept 2026. **No code is shared between Doom3Quest and RBDOOM-3-BFG** — they're two independent forks of two different id GPL releases (original Doom 3 2011 vs. BFG Edition 2012) that never converged. So "Doom 3 already had VR" is true, but specifically via the *other* lineage (dhewm3), not the one we've been rendering-testing.

**And the Carmack/Rift history, for the record:** confirmed (Engadget, Aug 2012; contemporaneous reporting) that Carmack built and publicly demoed a custom, internal stereoscopic/head-tracked Doom 3 BFG build at E3 and QuakeCon 2012 — first on his own prototype HMD, then on an early Oculus Rift unit, which is part of what led to him joining Oculus in Aug 2013. That specific code was never part of id's official Nov 2012 GPL source release (best available evidence: it targeted different, pre-DK1 hardware and became incompatible once the Rift DK1 shipped; no source documents it being merged into the public dump). Oculus/ZeniMax also later formally planned, then cancelled, official Doom 3 BFG VR support amid the ZeniMax v. Oculus lawsuit dispute — so the "first VR game" memory is accurate history, just a piece of code that was never open-sourced, not something we're missing by oversight.
...
## LithTech (Monolith) — checked against the same "clean license" bar (2026-09-16)

The user asked directly: Monolith's engine (LithTech, used from Blood 2 onward through F.E.A.R.) — is it released/open source? Checked source, not folklore:

- **LithTech 1.0 (Shogo: MAD, Blood 2: The Chosen)**: only the **game code** was released (Shogo Nov 1998, v2.1/v2.2 Dec 1998/Mar 1999; Blood 2 Apr 1999) — the **engine source itself was never released**. This is the mirror image of the original 1997 Blood/Build situation (there the engine was opened, the game code wasn't). Net effect: **Blood 2 is not a viable open engine base** — no full LithTech 1.0 source exists publicly.
- **LithTech Jupiter EX** (the branch used for **F.E.A.R.**, Condemned, and shared lineage with NOLF2/Tron 2.0): source code surfaced online around 2009, right as Touchdown Entertainment's site (Monolith's tools/licensing arm) went dark — described in community repos as a **GPL release, "PC Enterprise Edition Build 69."** Mirrored today in several GitHub forks (`pauliver/lithtech`, `poz1/LithTech`, `MWisBest/lithtech`, `Lastnote/lithtech`). Confirmed in `pauliver/lithtech`: NOLF2 (engine + game code) and Tron 2.0 (game code) are the complete, compiling parts; Shogo/NOLF/F.E.A.R./Blood2 directories exist but are flagged incomplete (e.g. Shogo doesn't compile). **Caveat carried over from the Quake4/Prey SDK case**: "GPL release" here is a community characterization of an engine dump, not id-style official GPL — worth a real license-text read before leaning on it, same as we did for the Quake4/Prey EULA above. Also: **no assets included**, only source — and the repo's own README flags unresolved differences between NOLF2's retail data files and what shipped with this release.
- **Conclusion for drap**: LithTech Jupiter EX is a real, technically-open C++ codebase, but it's a rough, largely unmaintained community mirror — no modern renderer work, no Vulkan, no active multi-year modernization effort like RBDOOM-3-BFG/TDM have. Not a near-term engine base; noted for completeness, not pursued further right now.

### F.E.A.R. VR — a genuinely relevant find (`DR-89/fear-vr`)

Separately from the engine-source question: **`DR-89/fear-vr`** is an actively developed (82 commits, at milestone "M6 — Packaging and regression" as of 2026-09-16), open-source (MIT for the mod's own code) **native OpenXR VR mod for F.E.A.R. 1.08**. Verified from its own README/THIRD_PARTY_NOTICES:

- **Real per-eye stereo, not a duplicated-mono hack**: hooks the existing LithTech Jupiter EX renderer via `d3d9.dll`/`dinput8.dll` proxy DLLs; the LithTech camera renders twice per frame with its own per-eye matrix, bridged to a separate x64 OpenXR host process over shared-memory IPC.
- **Legally clean distribution**, same pattern as this project's own approach to Quake3VR/RBDOOM-3-BFG/TDM: ships **no retail files, no extracted assets, no proprietary SDK source** — you must own F.E.A.R. 1.08 yourself (Ultimate Shooter Edition on Steam confirmed working) plus the official Public Tools 1.08 SDK installer.
- **Windows-only**: x86 D3D9 bridge + x64 OpenXR host, Visual Studio 2022 builds, no Linux build — doesn't fit the `reverb-g2`/Monado-on-Linux stack natively the way Quake3VR/Trinity VR's `.deb` does; would need a Proton/Wine test to know if it even applies to our hardware setup, not assumed to work.
- **Confirmed on real hardware by its own project** (Quest 3 via SteamVR/VirtualDesktopXR) — a working data point, not vaporware, same bar this doc has been holding everything else to.

**Where this fits**: same category as Doom3Quest/PreyVR/Fully Possessed — a VR *mod* riding on a proprietary game you must own, not an open engine to build on — but with a real edge over Doom3Quest: it targets **desktop OpenXR directly**, not Android-first like Doom3Quest/PreyVR. Worth keeping as a reference/comparison point alongside the idTech4 and Quake3 fronts; the one hard mismatch against everything else in this doc is that it's Windows-only against a Linux-first hardware bar.

## Upstream news check (2026-09-17, iashur unreachable — rebooted to Windows)

`iashur` is on Windows right now, so no headset diagnostic today. Checked upstream activity on every project this doc leans on instead, using real commit/API dates, not READMEs' static status text:

- **Monado — real lead for the black-panel bug.** Very active upstream. Yesterday (2026-09-16) `master` merged **MR !2993** ("Do bounds checking for projection layers in compute compositor") — fixes the **compute compositor's layer squasher** sampling out-of-bounds UVs during timewarp reprojection when transformed view coordinates fall outside the view frustum; explicitly notes "the graphics compositor already handles this correctly." Landed right after our 2026-09-15 test that ended on an unexplained black HMD panel with a healthy desktop mirror and a real swapchain. **Not confirmed as the same bug** (that MR describes corrupted/garbage sampling, not necessarily a fully blank panel) — but it's the right subsystem (projection-layer/timewarp path) at the right time, so `git pull && rebuild` on Monado is now a cheap first step before the `RUST_LOG=xrizer=trace` diagnostic that was already queued.
- **Trinity VR (Quake3VR/ioquake3 lineage) — healthy, active project.** Commits as recent as today (2026-09-16 23:19 UTC): frustum-culling fixes, update-system work, a pinned Trinity matchmaking version. The DM VR front we already got running at a clean 90Hz remains a live upstream, not an orphaned fork.
- **xrizer upstream — effectively dead.** `The-personified-devil/xrizer`'s `main` branch has had **no commits since 2025-01-04** (confirmed via `git fetch` + `git log`, not just a webpage read). The `wait_image`/`release_image` fix from 2026-09-15 has nowhere to be upstreamed to — it stays a permanent local patch in the user's own checkout, not something to expect review or a merge for. Worth knowing before planning around "eventually this gets fixed upstream."
- **RBDOOM-3-BFG mainline — active, nothing VR-related.** Real merges as recent as 2026-08-07 (Vulkan/Wayland fullscreen fix, shadow-map fixes), but no VR/OpenXR/OpenVR work merged. Checked every VR-named branch on the official repo (`openvr2` through `openvr5`, `DOOM-3-BFG-VR-sync`, `mods/samsonvr`): **the newest of them (`openvr5`) hasn't been touched since 2025-09-13, over a year stale.** Confirms what this doc already concluded — VR-on-RBDOOM-3-BFG is genuinely frontier work we're doing ourselves on `rbdoom3bfg-vr`, not catching up to something already further along upstream.
- **F.E.A.R. VR (`DR-89/fear-vr`) — correction to the 2026-09-16 read.** Checked real push/release dates (not the README's static "M6" status text): **last push and last release (`v1.0.0-beta.8`) were both 2026-08-04** — quiet for over five weeks, not the "actively being built right now" impression the milestone label gave. Doesn't change the earlier recommendation (Windows-only, not pursued), just corrects the activity read.
- **dhewm3 PR #771 (render/sim decoupling)** — still open, unmerged, real review activity into September: a tester found a **crash during the "invasion" scene on Windows**, and the author is bisecting with a minimal build that drops interpolation to isolate the cause. Still not close to merge-ready; the conclusion that RBDOOM-3-BFG's `com_engineHz` is the more solid path for drap stands.

**Concrete next step once `iashur` is back on Linux:** `git -C ~/vr/monado pull && rebuild`, then retry `RUST_LOG=xrizer=trace` on the RBDoomVR black-panel bug before any other diagnostic — cheapest test that could plausibly already be fixed by someone else's work.
